...
// check the username in the user table and get the matching user_id
			...
			// get the review_id in the review table, then get the matching fields in the row 
			   ...
			   //the other value in the category table, cat_id, is auto incremented, so it is inserted automatically
				...
				$stmt5 = $con->prepare("UPDATE category SET cat_name=? WHERE Review_id=?") or die(mysqli_error($con));
				$stmt5->bind_param('s', $category, $name, $phone, $address, $comment, $Review_id ) or die ("MySQLi-stmt binding failed ".$stmt2->error);
				$stmt5->execute() or die ("MySQLi-stmt execute failed ".$stmt2->error);
